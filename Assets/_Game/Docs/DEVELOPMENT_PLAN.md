...
Пройдено: **этап 0**, офлайн **2–4**, полировка **6.1–6.3** и **6.6**, сервер **5.0–5.10**, аккаунт **1.1–1.8**.  
**Только что:** фикс Casual — скилл и возврат на сервере; лагеря леса в MatchLoop; заголовок Results при сдаче (победа / поражение); JungleNorth/South — декали без тонких кубов (на телефоне больше не «полка» над полом).

**Дальше:** пересобрать плагин на VPS (`server/match/` целиком). Проверка Casual: Q и Возврат, монстры в лесу с HP, сдача → победителю «Победа · сдача». Затем **6.4** баланс и **6.5** туториал. Магазин фонтана в сети ещё локальный (не в этом фиксе). Настоящие модели карты заменят примитивы.
...
| **5.7** | Подключение только после Match Found | ✅ Hall — мета HTTP; realtime сокет с очереди; JoinMatch после Match Found |
| **5.8** | Disconnect / Leave после Results | ✅ Continue → `DisconnectRealtimeAsync` |
...
До **6.4** (баланс) casual-бой полностью на сервере: герои, волны, турель, кристалл, предикт, реконнект 30 с, пинг союзникам. SOLO — офлайн как раньше.
